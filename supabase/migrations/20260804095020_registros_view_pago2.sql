...
select
  ...
  case
    when current_user_role() = any (array['superadmin'::user_role, 'admin'::user_role]) then pago1_forma_pago
    else null::payment_method
  end as pago1_forma_pago,
  case
    when current_user_role() = any (array['superadmin'::user_role, 'admin'::user_role]) then pago1_monto
    else null::numeric
  end as pago1_monto,
  case
    when current_user_role() = any (array['superadmin'::user_role, 'admin'::user_role]) then pago2_forma_pago
    else null::payment_method
  end as pago2_forma_pago,
  case
    when current_user_role() = any (array['superadmin'::user_role, 'admin'::user_role]) then pago2_monto
    else null::numeric
  end as pago2_monto
